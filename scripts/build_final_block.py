...
E=html.escape
# Reuse the approved diagram renderer without running the preview builder.
tree=ast.parse((REPO/'scripts/build_video.py').read_text())
exec(compile(ast.Module(body=[n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='visual'],type_ignores=[]),'visual','exec'))
...
def build(lang,part):
 ...
 lesson=json.loads((REPO/f'docs/lesson-{lang}.json').read_text());scenes=[lesson[i-1] for i in block['scenes']]
 ...
 dest=ROOT/'final'/key;(dest/'assets').mkdir(parents=True,exist_ok=True);(dest/'compositions').mkdir(exist_ok=True)
 for name in ['display.ttf','mono.ttf','gsap.min.js']:
  shutil.copy2(ROOT/'assets'/name,dest/'assets'/name)
 target=dest/'assets/avatar.mp4'
 if not target.exists():target.symlink_to(ROOT/'assets'/f'nei-{key}.mp4')
...
'''
 ...
 for i,s in enumerate(scenes):
  ...
  original=(ROOT/f'compositions/{ident}.html').read_text()
  ...
  script=re.sub(r'const a=1\.8\+i\*[\d.]+',f'const a=1.8+i*{max(.5,(length-4)/max(len(s["labels"]),1))}',script)
  markup=f'''<!doctype html><html lang="{lang}"><body><template><style>#{ident}-root{{position:absolute;inset:0;width:100%;height:100%;}}</style><div id="{ident}-root" data-composition-id="{ident}" data-width="1920" data-height="1080" data-duration="{length}"><div class="scene-shell"><header class="topline"><span>{E(s['chapter'])}</span><span>INEMA · {localization[0]}</span></header><div class="layout"><main><h1 class="main-title">{E(s['title'])}</h1><div class="visual kind-{s['kind']}">{visual(s).replace('Cenário didático · entrada por solicitação · sem cache',localization[3])}</div></main><aside class="avatar-panel"><p class="avatar-name">Nei Maldaner</p><div class="avatar-desc">{localization[2]}</div></aside></div><div class="takeaway">{E(s['takeaway'])}</div><footer class="footer"><span>GPT-6 ASTRA · {localization[1]}</span><span>{number:02d} / 22</span></footer><div class="progress-track"><div class="progress-fill"></div></div></div></div><script>{script}</script></template></body></html>'''
