...
ROOT=Path('/home/nmaldaner/projetos/output/astrabasico');STATE=ROOT/'verification/production.json'
...
for attempt in range(720):
 ...
 if len(state)==13 and all(x.get('status') in ['rendered','failed'] for x in state.values()):break
 time.sleep(60)
