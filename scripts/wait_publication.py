...
for attempt in range(720):
 if all((ROOT/f'verification/assembled-{lang}.json').exists() for lang in ['pt','es','en']):
  ...
  publish()
  subprocess.run(['node','scripts/send_final_v3.mjs'],cwd=REPO,check=True)
  (ROOT/'STATUS.md').write_text('# Astra Básico — concluído\n\nTrês vídeos completos, MP4 e SRT, publicados na release v1.1.0. Guia atualizado por git e links enviados ao bot v3.\nhttps://inematds.github.io/astrabasico/guia/\nhttps://github.com/inematds/astrabasico/releases/tag/v1.1.0\n')
  break
 time.sleep(60)
